...
寄附ページのヘロセクションは、ユーザーが最初に目にする重要な領域です。ただし以下の課題がありました：
...
3. **CTA と バッジの情報設計が未最適化**：寄附前の不安感（「本当に安全か？」「後で取り消せるか？」）の即時払拭が不十分
...
| 項目 | 変更前 | 変更後 | 理由 |
|------|--------|--------|------|
| H1 | 寄附のご案内 | Discordコミュニティの運営を支えるための寄附 | 主人公性・行動喚起を強化 |
