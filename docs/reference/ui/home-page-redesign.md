...
- `.page-enter` は **下方向 48px から上方へスライド**しながらフェードインする演出。複数要素を順番に表示したい場合は、各コンテナへ `style={{ '--sequence-index': <n> }}` を指定すると `0.08s` 刻みで遅延が付与される。
- `prefers-reduced-motion: reduce` が有効な環境ではアニメーションをスキップし、即時表示となる。
